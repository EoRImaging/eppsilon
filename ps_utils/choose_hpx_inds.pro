pro choose_hpx_inds, cube_file, inds_save_file, default_size_multiple = default_size_multiple, $
  ...
  if strpos(obs.instrument, 'mwa') ge 0 then begin
    default_size_meters = 4.5 ;use 4.5m to be conservative
  endif else begin
    if strpos(obs.instrument, 'hera') ge 0 then begin
      default_size_meters = 10.5 ;calculated by Dara from the beam model at her central frequency
    endif else begin
      message, "unknown instrument: " + obs.instrument + ". Cannot set default_size_meters"
    endelse
  endelse
  ...
  ; get the pixel closest to the phase center.
  ; Theta is latitude measured southward from north, {0, pi}
  hpx_theta = !dpi/2.-obs.obsdec*!dpi/180.
  ; phi is longitude measured eastward {0,2pi}
  hpx_phi = obs.obsra*!dpi/180.
  
  vec2ang, vec_mid, dec_mid, ra_mid,/astro
  print, "vector midpoint corresponds to RA: " + string(ra_mid) + ", Dec: " + string(dec_mid)
  print, "phase center from obs structure is RA: " + string(obs.obsra) + ", Dec: " + string(obs.obsdec)
  ang2vec, obs.obsdec, obs.obsra, phase_center_vec, /astro
  ...
  if keyword_set(show_plot) then cgplot, x_rot, y_rot, psym=3
  if keyword_set(show_plot) then cgplot, x_list_rot, y_list_rot, psym=3, color="blue", /over
