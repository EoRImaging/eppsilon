function image_mask_eppsilon, x_rot, y_rot, wh_close, filter_name=filter_name,old_code=old_code
  ...
  if filter_name NE 'none' then mask_bh = spectral_window(1000, type = filter_name, periodic = periodic) else  begin
    mask_bh = FLTARR(1000)
    mask_bh[*]=1.
  endelse
  ...
  save, pix_mask, filename = '/nfs/mwa-00/h1/nbarry/UVfromFHD/pix_mask_'+filter_name+'.sav'
  return, pix_mask
end