pro ps_wrapper
  ...
  ;; the following 2 lines sets the save_path to a 'ps' directory inside the datafile directory
  temp = strpos(datafile, '/', /reverse_search)
  save_path = strmid(datafile, 0, temp+1) + 'ps/'
  ...
  ;; There are 3 refresh keywords to indicate that various stages should be re-calculated 
  ...
  ;; The earliest refresh keyword is refresh_dft, which is only for Healpix datasets (it's ignored otherwise)
  ;; The next stage is refresh_ps and the last stage is refresh_binning.
  ;; To set any of these keywords, set them equal to 1 (true)

  ;; refresh_dft=1


  ;; options for binning:
  ;; log_kperp, log_kpar and log_k1d are flags: set to 1 (true) for logrithmic bins
  ...
  ;; data_range specifies the min & max value of the plot colorbar
  ;; (values outside that range are clipped to those values)
