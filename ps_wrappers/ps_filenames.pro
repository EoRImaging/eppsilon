  ...
  if keyword_set(rts) then begin
    ...
    for i=0, n_filesets-1 do begin
      ...
      if keyword_set(exact_obsnames) then begin
        if keyword_set(uvf_input) then info_file = file_search(save_paths[i] +  'Combined_obs_' + obs_names[i] + uvf_endobsname_tag + uvf_info_tag + '*info*', count = n_infofile) $
        else begin
    ...
    if n_infofile gt 0 then begin
      if obs_names[i] eq '' then begin
        ...
        if n_infofile gt 1 then begin
          ...
          if keyword_set(uvf_input) then new_info_file = file_search(save_paths[i] +  'Combined_obs_' + obs_names[i] + uvf_endobsname_tag + uvf_info_tag + '*info*', count = n_infofile) $
          else begin
          hpx_info_file = file_search(save_paths[i] +  'Combined_obs_' + obs_names[i] + hpx_endobsname_tag + '*info*', count = n_hpx)
          uvf_info_file = file_search(save_paths[i] +  'Combined_obs_' + obs_names[i] + uvf_endobsname_tag + uvf_info_tag + '*info*', count = n_uvf)
    ...
    if keyword_set(exact_obsnames) then begin
      if keyword_set(uvf_input) then info_file = file_search(save_paths[i] + obs_name_single + uvf_endobsname_tag + uvf_info_tag + '*info*', count = n_infofile) $
      else begin
