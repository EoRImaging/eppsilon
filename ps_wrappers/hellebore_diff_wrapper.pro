pro hellebore_diff_wrapper, folder_names, cube_types = cube_types, pols = pols, $
  ...
  if n_elements(save_path) gt 0 then begin
    ...
    if n_elements(folder_names) eq 2 then begin
      ...
      if count_diff eq 0 then save_path_use = file_dirname(info_file, /mark_directory) else begin
        ...
        if count_name_diff eq 0 then diff_dir = file_basename(folder_names[0]) + '_diff' else $
          diff_dir = strjoin(fnameparts_1[wh_name_same], '_') + '__' + strjoin(fnameparts_1[wh_name_diff]) + '_minus_' + strjoin(fnameparts_2[wh_name_diff])
          
        save_path_use = joint_path + path_sep() + diff_dir + path_sep()
      endelse
      if file_test(save_path_use) eq 0 then file_mkdir, save_path_use
    endif else save_path_use = file_dirname(info_file, /mark_directory)
    
    pos = strpos(save_path_use, std_savepath)
    if pos ne -1 then save_path_ext = strmid(save_path_use, pos + strlen(std_savepath)) $
    else save_path_ext = ''
  endelse
  ...
  if n_elements(folder_names) eq 1 then plot_filebase = fhd_types[0] + '_' + cube_types[0] + '_' + pols[0] + $
    ...
    plot_filebase = fhd_types[0] + '_' + cube_types[0] + '_' + pols[0] + $
