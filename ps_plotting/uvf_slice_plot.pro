...
pro uvf_slice_plot, slice_savefile, uvf_slice=uvf_slice, xarr=xarr, yarr=yarr, $
  ...
  if keyword_set(log) then begin
    ...
    log_color_calc, plot_slice, slice_plot_norm, cb_ticks, cb_ticknames, $
      color_range, n_colors, data_range = data_range, $
      color_profile = color_profile, log_cut_val = log_cut_val, oob_low = oob_low

    if keyword_set(all_zero) then slice_plot_norm = slice_plot_norm * 0
  ...
  if not keyword_set(no_title) then begin
    ...
    if n_elements(multi_pos) gt 0 then begin
      yloc_title = plot_pos[3] + 0.7* (multi_pos_use[3]-plot_pos[3])
    endif else begin
  ...
  axkeywords = {xstyle: 5, ystyle: 5, thick: thick, charthick: charthick, $
    xthick: xthick, ythick: ythick, charsize: charsize, font: font}

  cgimage, slice_plot_norm_rgb, /nointerp, position = plot_pos, $
    xrange = minmax(plot_xarr), yrange = minmax(plot_yarr), $
    title = initial_title, noerase = no_erase, color = annotate_color, $
  ...
  if keyword_set(mark_0) then begin
    ...
    if slice_axis eq 2 then begin
      cgplot, /overplot, plot_xarr, plot_xarr* 0d, color=annotate_color, psym=-3, thick = thick
    endif
  endif
  ...
  if type eq 'phase' then begin
    cb_range = data_range * 180/!dpi
  endif else begin
    cb_range = data_range
  endelse
  ...
  if keyword_set(log) then begin
    ...
    cgcolorbar, color = annotate_color, /vertical, position = cb_pos, $
      charsize = charsize, font = font, minrange = cb_range[0], $
      maxrange = cb_range[1], title = cb_title, minor=5, charthick = charthick, $
