...
pro kpower_slice_plot, slice_savefile, power = power, noise = noise, $
    noise_expval = noise_expval, weights = weights, xarr = xarr, yarr = yarr, $
    ...
    no_units = no_units, pwr_ratio = pwr_ratio, charsize_in=charsize_in, kz_mpc=kz_mpc
    ...
    if n_elements(plot_2d_options) gt 0 then begin
      ...
      if n_elements(plot_xrange) eq 0 and tag_exist(plot_2d_options, 'kperp_plot_range') then begin
        plot_xrange = plot_2d_options.kperp_plot_range
      endif
    endif
  ...
  if keyword_set(pwr_ratio) then begin
    if n_elements(power) eq 0 then begin
      ...
      if total(abs(yarr - getvar_savefile(slice_savefile[1], 'yarr'))) ne 0 then begin
        message, 'yarr do not match in savefiles'
      endif
      slice_axis = getvar_savefile(slice_savefile[0], 'slice_axis')
      if total(abs(slice_axis - getvar_savefile(slice_savefile[1], 'slice_axis'))) ne 0 then begin
        message, 'slice_axis do not match in savefiles'
      endif
      slice_inds = getvar_savefile(slice_savefile[0], 'slice_inds')
      if total(abs(slice_inds - getvar_savefile(slice_savefile[1], 'slice_inds'))) ne 0 then begin
        message, 'slice_inds do not match in savefiles'
      endif
      kperp_lambda_conv = getvar_savefile(slice_savefile[0], 'kperp_lambda_conv')
      if total(abs(kperp_lambda_conv - getvar_savefile(slice_savefile[1], 'kperp_lambda_conv'))) ne 0 then begin
          message, 'kperp_lambda_conv do not match in savefiles'
      endif
      delay_params = getvar_savefile(slice_savefile[0], 'delay_params')
      if total(abs(delay_params - getvar_savefile(slice_savefile[1], 'delay_params'))) ne 0 then begin
        message, 'delay_params do not match in savefiles'
      endif
      hubble_param = getvar_savefile(slice_savefile[0], 'hubble_param')
      if total(abs(hubble_param - getvar_savefile(slice_savefile[1], 'hubble_param'))) ne 0 then begin
  ...
  if n_elements(plot_yrange) eq 0 and n_elements(plot_2d_options) gt 0 then begin
    if slice_axis eq 2 then begin
      if tag_exist(plot_2d_options, 'kperp_plot_range') then begin
        plot_yrange = plot_2d_options.kperp_plot_range
      endif
    endif else begin
      if tag_exist(plot_2d_options, 'kpar_plot_range') then begin
        plot_yrange = plot_2d_options.kpar_plot_range
      endif
    endelse
  endif
  ...
  if n_elements(plot_xrange) eq 0 then begin
    ...
    if keyword_set(linear_axes) then begin
      ...
      plot_xrange = [0, max(xarr_edges[[temp, max(temp)+1]])]
    endelse
  endif
  ...
  if not keyword_set(no_title) then begin
    ...
    if n_elements(multi_pos) gt 0 then begin
      yloc_title = plot_pos[3] + 0.8* (multi_pos_use[3]-plot_pos[3])
    endif else begin
  ...
  if keyword_set (hinv) then begin
    plot_xtitle = textoidl('k_' + plot_xname + ' (!8h!X Mpc^{-1})', font = font)
  endif else begin
    plot_xtitle = textoidl('k_' + plot_xname + ' (Mpc^{-1})', font = font)
  endelse
  if keyword_set (hinv) then begin
    plot_ytitle = textoidl('k_' + plot_yname + ' (!8h!X Mpc^{-1})', font = font)
  endif else begin
    plot_ytitle = textoidl('k_' + plot_yname + ' (Mpc^{-1})', font = font)
  endelse
  ...
  if keyword_set(plot_wedge_line) and slice_axis lt 2 then begin
    ...
    for i=0, n_lines-1 do begin
      cgplot, /overplot, plot_xarr, abs(plot_xarr * wedge_amp[i]), color = annotate_color, $
      thick = thick+1, psym=-0, linestyle = linestyles[i]
    endfor
  endif
  ...
  if keyword_set(baseline_axis) then begin
    ...
    if keyword_set(hinv) then begin
      baseline_range = minmax(plot_xarr * hubble_param * kperp_lambda_conv)
    endif else begin
      baseline_range = minmax(plot_xarr* kperp_lambda_conv)
    endelse
    ...
    cgtext, xloc_lambda, yloc_lambda, textoidl('(\lambda)', font = font), /normal, $
      alignment=0.5, charsize=charsize, color = annotate_color, font = font
  endif else begin
    cgaxis, xaxis=1, xrange = minmax(plot_xarr), xtickv = xticks, $
      xtickname = replicate(' ', n_elements(xticks)), charthick = charthick, $
      xthick = xthick, ythick = ythick, charsize = charsize, font = font, xstyle = 1, $
  ...
  if keyword_set(baseline_axis) and slice_axis eq 2 then begin
    ;; baselines don't care about hinv -- take it back out.
    if keyword_set(hinv) then begin
      baseline_range = minmax(plot_yarr * hubble_param * kperp_lambda_conv)
    endif else begin
      baseline_range = minmax(plot_yarr* kperp_lambda_conv)
    endelse
    ...
    cgtext, xloc2_lambda, yloc2_lambda, textoidl('(\lambda)', font = font), /normal, $
      alignment=0.5, charsize=charsize, color = annotate_color, font = font
  endif else begin
    if keyword_set(delay_axis) or keyword_set(cable_length_axis) then begin
      ...
      cgaxis, yaxis=1, yrange = yrange_use, ytickformat = 'exponent', $
        charthick = charthick, xthick = xthick, ythick = ythick, charsize = charsize, $
        font = font, ystyle = 1, color = annotate_color

      cgtext, xloc2_lambda, yloc2_lambda, units_text, /normal, alignment=0.5, $
        charsize=charsize*0.9, color = annotate_color, font = font

    endif else begin
      cgaxis, yaxis=1, yrange = minmax(plot_yarr), ytickv = yticks, $
  ...
  cgcolorbar, color = annotate_color, /vertical, position = cb_pos, $
    bottom = color_range[0], ncolors = n_colors, minor = 0, $
